...
module.exports = {
  ...
  if (!player) {
    let embed = new MessageEmbed()
      .setColor("ff0000")
      .setDescription("<:Error:939761035164921906>| There is **0** songs in queue")
  ...
  if (songs === null) {
    ...
    return message.channel.send({content:` \`NOW PLAYING: ${player.queue.current.title}\` \`\`\`css\n${songsForPageContent}\`\`\` `})
  }
  ...
  const msg = await message.channel.send({content:`\` **NOW PLAYING**: ${player.queue.current.title}\` \`\`\`css\n${songsForPageContent.join('\n')}\`\`\` `})
  await msg.react('⬆️')
  await msg.react('⬇️')
  ...
  function forward() {
    ...
    msg.edit({content:`\`NOW PLAYING: ${player.queue.current.title}\` \`\`\`css\n${songsForPageContent.join('\n')}\`\`\` `})
  }
